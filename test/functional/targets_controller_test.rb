...
class TargetsControllerTest < ActionController::TestCase
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post :create, target: { description: @target.description, dictionary_url: @target.dictionary_url, endpoint_url: @target.endpoint_url, graph_uri: @target.graph_uri, ignore_predicates: @target.ignore_predicates, max_hop: @target.max_hop, name: @target.name, parser_url: @target.parser_url, sample_queries: @target.sample_queries, sortal_predicates: @target.sortal_predicates }
    end
